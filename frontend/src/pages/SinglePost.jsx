    ...
    try
    {
      await axios.delete(`${baseUrl}/deletePost/${params.id}`).then((res)=>{
        if(res.data.message === 'Not authenticated')
        {
          ...
          alert(res.data.message)
        } else if(res.data.message === "This post doesn't belong to you or does not exist"){
          alert(res.data.message)
        }else{
          alert(res.data.message)
          navigate('/')
        ...
        <div className='w-[100%]  flex md:flex-row gap-11 flex-col'>
            {/* content */}
              <div className="md:w-[70%] w-full">
                {/* user */}
                  <img className='w-full md:h-auto rounded-md object-cover'
                  src={`../upload/${post.postImg}`} alt="" />
                    ...
                    isLoggedIn ? (user.username === post.username ? (
                      ...
                      <div className='flex gap-2'>
                          ...
                          <div className='flex items-center gap-1 -mt-5'>
                            ...
                            <img onClick={handleDelete} className='cursor-pointer w-6 h-6' src={deleteLogo} alt="" />
                          </div>
              ...
              {/* product info */}
                <div className='mt-9'>
                  <h1 className='font-bold text-2xl mb-9'>
                    {post.title}
                  </h1>
                    <div className='md:text-xl text-lg post-description overflow-x-hidden overflow-y-auto whitespace-pre-wrap break-words text-justify'
