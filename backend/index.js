...
app.get('/post/:id',(req,res)=>{
    ...
    const q = 'select views,p.id,username,title,description,cat,date,p.img as postImg,u.img as userImg from users as u join posts as p on u.id = p.user_id where p.id = ?'
    ...
    jwt.verify(token, secret, (err, userInfo) => {
        ...
        db.query(q, [postId, userId], (err, result) => {
            ...
            if (result.affectedRows === 0) {
                return res.status(403).json({ message: "This post doesn't belong to you or does not exist" });
            }
...
app.post('/add-post',(req,res)=>{
    ...
    jwt.verify(token, secret, (err, userInfo) => {
        if (err)
        {
            ...
            const q = 'insert into posts (`title`, `description`,`img`,`date`,`user_id`,`cat`) values(?)'
            const values = [
                req.body.title,
